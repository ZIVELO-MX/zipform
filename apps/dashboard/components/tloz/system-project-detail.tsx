...
export function SystemEntityDetail(props: DetailProps & { panel?: boolean; onChange?: (entity: TlozProject | TlozQuestItem) => void; onNavigateMission?: (mission: TlozMissionRecord) => void; onOpenFullPage?: () => void }) {
  ...
  return <article className={`mission-detail-workspace ${panel ? "px-5 py-6" : "mx-auto max-w-[1052px] px-[26px] py-7"} min-h-full bg-[#FAFAF9]`}>
    ...
    <div className="mission-detail-layout grid min-w-0 gap-[30px]">
      <main className="min-w-0">
        ...
        <EditableText value={entity.description} multiline placeholder="Añadir descripción…" className="mb-7 block min-h-24 w-full text-[15px] leading-[1.6] text-[#454543]" onSave={(description) => persist({ description })} />

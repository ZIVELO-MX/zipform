...
import { Breadcrumb, BreadcrumbItem, BreadcrumbLink, BreadcrumbList, BreadcrumbPage, BreadcrumbSeparator, Button, ColorPicker, DatePicker, displayUsername, DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, IconPicker, Select, SelectContent, SelectGroup, SelectItem, SelectTrigger, SelectValue, SlideOver, toast, useOverlayToasterId, UserAvatarLabel, UserPicker, type IconPickerOption } from "@zipform/ui";
...
function ResourceCard({ resource, onRemove }: { resource: TlozResource; onRemove: () => void }) { const Icon = resolveResourceIcon(resource); const content = <><span className="grid size-7 shrink-0 place-items-center rounded-lg bg-[#EEF2FF] text-[#3A47B5]"><Icon size={13} /></span><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{resource.title}</strong><span className="block truncate text-xs text-carbon/45">{resourceTypeLabel[resource.type]}</span></span></>; return <div className="group flex items-center gap-2 rounded-xl border border-carbon/10 bg-white px-3 py-3">{resource.url ? <a className="flex min-w-0 flex-1 items-center gap-3 text-carbon no-underline" href={resource.url} target="_blank" rel="noreferrer">{content}</a> : <div className="flex min-w-0 flex-1 items-center gap-3">{content}</div>}<Button type="button" variant="ghost" size="icon-xs" aria-label={`Eliminar ${resource.title}`} onClick={onRemove}><X /></Button></div>; }

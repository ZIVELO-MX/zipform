...
export function MissionDetailPage({ mission, options, canUpdate }: { mission: TlozMissionDetail; options: MissionDetailOptions; canUpdate: boolean }) {
  ...
  return <>
    <DocumentDetail mission={mission} options={options} canUpdate={canUpdate} onNavigateQuestItem={(id) => setSelectedItem(options.questItems.find((item) => item.id === id) ?? null)} />

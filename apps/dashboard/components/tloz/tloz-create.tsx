...
function MissionRelations({ draft, field, missions, questItems, resources, setResources }: { draft: Record<string, string>; field: (name: string, value: string) => void; missions: TlozMissionRecord[]; questItems: TlozQuestItem[]; resources: TlozResourceInput[]; setResources: React.Dispatch<React.SetStateAction<TlozResourceInput[]>> }) {
  ...
  return <div className="flex flex-col gap-5"><CreateSection title="Dependencias"><div className="flex flex-col gap-2">{dependencyIds.map((id) => <SelectedRelation key={id} label={missions.find((item) => item.id === id)?.title ?? id} onRemove={() => remove("dependencyIds", id)} />)}<AddDependency relationType="mission" missions={missionOptions.filter((item) => !dependencyIds.includes(item.id))} questItems={[]} onAddMission={(id) => add("dependencyIds", id)} onAddQuestItem={() => undefined} /></div></CreateSection><CreateSection title="Quest Items"><div className="flex flex-col gap-2">{questIds.map((id) => <SelectedRelation key={id} label={questItems.find((item) => item.id === id)?.name ?? id} onRemove={() => remove("requiredQuestItemIds", id)} />)}<AddDependency relationType="quest" missions={[]} questItems={questOptions.filter((item) => !questIds.includes(item.id))} onAddMission={() => undefined} onAddQuestItem={(id) => add("requiredQuestItemIds", id)} /></div></CreateSection><CreateSection title="Attachments / Resources"><div className="flex min-w-0 flex-col gap-2">{resources.map((resource, index) => <SelectedRelation key={`${resource.title}-${index}`} label={resource.title || "Nuevo recurso"} onRemove={() => setResources((items) => items.filter((_, itemIndex) => itemIndex !== index))} />)}<AddResource onAdd={(resource) => setResources((items) => [...items, resource])} /></div></CreateSection></div>;
}

...
import { formatDate, missionStatusLabel, missionTypeLabel, missionTypeTone } from "./tloz-utils";
...
export function MissionInlineEditor({ mission, options, onMissionChange }: { mission: TlozMissionRecord; options?: MissionEditorOptions; onMissionChange?: (mission: TlozMissionRecord) => void }) {
  ...
  return <div className="flex flex-col" aria-busy={isPending}>
    <DetailPropertyRow label="Estado" display={<StatusValue status={current.status} />} editing={activeField === "status"} onEdit={(editing) => setActiveField(editing ? "status" : null)}>
    ...
    <DetailPropertyRow label="Categoría" display={(() => { const tone = missionTypeTone[current.type]; return <span className="inline-flex rounded-full px-[9px] py-[3px] text-xs font-bold" style={{ background: tone === "#d72228" ? "#FDECEC" : tone === "#2d6cdf" ? "#EEF2FF" : tone === "#1e8e5a" ? "#E6F4EA" : "#F2EAFE", color: tone }}>{missionTypeLabel[current.type]}</span>; })()} editing={activeField === "type"} onEdit={(editing) => setActiveField(editing ? "type" : null)}>
    ...
    {options?.users.length ? <DetailPropertyRow label="Responsable" display={<UserAvatarLabel name={current.owner.name} label={current.owner.username ?? current.owner.name} labelOnly imageUrl={current.owner.avatarUrl} size="sm" />} editing={activeField === "owner"} onEdit={(editing) => setActiveField(editing ? "owner" : null)}>
    ...
    <DetailPropertyRow label="Proyecto" display={<span className="inline-flex min-w-0 items-center gap-1.5"><span className="size-[7px] shrink-0 rounded-full" style={{ backgroundColor: current.project?.color ?? "#9a9a98" }} /><span className="truncate">{current.project?.name ?? "Sin proyecto"}</span></span>} editing={activeField === "project"} onEdit={(editing) => setActiveField(editing ? "project" : null)}>
    ...
    <DetailPropertyRow label="Inicio" display={<span className="font-mono text-[12.5px] font-semibold">{formatDate(current.startDate)}</span>} editing={activeField === "startDate"} onEdit={(editing) => setActiveField(editing ? "startDate" : null)}>
      <DatePicker value={current.startDate} label="Fecha de inicio" onValueChange={(value) => persist({ startDate: value ?? "" }, "Fecha actualizada")} />
    </DetailPropertyRow>
    <DetailPropertyRow label="Vence" display={<span className="font-mono text-[12.5px] font-semibold text-[#B91C22]">{formatDate(current.dueDate)}</span>} editing={activeField === "dueDate"} onEdit={(editing) => setActiveField(editing ? "dueDate" : null)}>

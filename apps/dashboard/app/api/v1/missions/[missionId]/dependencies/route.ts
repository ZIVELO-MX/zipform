...
export async function POST(request: Request, { params }: { params: Promise<{ missionId: string }> }) {
  ...
  let body: { dependsOnMissionId?: string };
  try {
  ...
  if (!body.dependsOnMissionId) {
    return NextResponse.json(
      { error: { code: "INVALID_REQUEST", message: "dependsOnMissionId es requerido.", requestId: crypto.randomUUID() } },
      { status: 400 }
    );
  }

...
export function EntityPicker({ options, value, onValueChange, onCreate, label, triggerLabel, triggerVariant = "outline", disabled, allowEmpty = true, className }: {
  ...
  async function create() {
    ...
    if (disabled || !onCreate || !name || createPendingRef.current) return;
  ...
  function handleOpenChange(nextOpen: boolean) {
    if (!nextOpen && createPendingRef.current) {
      setOpen(true);
      return;
    }
    setOpen(nextOpen);
  ...
  function selectOption(id: string) {
    if (disabled || createPendingRef.current) return;
    onValueChange(id);
    setOpen(false);
    setQuery("");
  }
  ...
  return <Popover open={open} onOpenChange={handleOpenChange}>
    ...
    <PopoverContent className="w-[min(360px,calc(100vw-32px))]">
      ...
      <label className="relative block"><span className="sr-only">Buscar {label.toLocaleLowerCase("es")}</span><Search className="pointer-events-none absolute left-3 top-1/2 size-3.5 -translate-y-1/2 text-carbon/40" aria-hidden="true" /><Input disabled={disabled || creating} value={query} onChange={(event) => { setQuery(event.target.value); setCreateError(null); }} onKeyDown={(event) => {
        if (event.key !== "Enter" || event.nativeEvent.isComposing) return;
        event.preventDefault();
        if (disabled || creating || !normalizedQuery || createPendingRef.current) return;
        const exact = options.find((option) => option.name.toLocaleLowerCase("es") === normalizedQuery);
        const option = exact ?? filtered[0];
        if (option) {
          selectOption(option.id);
        } else if (onCreate) {
          void create();
        }
      }} placeholder={`Buscar ${label.toLocaleLowerCase("es")}…`} className="pl-9" autoComplete="off" /></label>
      <div className="mt-2 flex max-h-64 flex-col gap-1 overflow-y-auto overscroll-contain">
        {allowEmpty ? <button type="button" disabled={disabled || creating} className="flex min-h-10 items-center gap-2 rounded-lg px-2.5 text-left text-sm text-carbon/55 hover:bg-carbon/5" onClick={() => { onValueChange(""); setOpen(false); setQuery(""); }}><span className="grid size-7 shrink-0 place-items-center rounded-lg bg-carbon/5"><X className="size-3.5" aria-hidden="true" /></span>Sin {label.toLocaleLowerCase("es")}</button> : null}
        {allowEmpty && filtered.length ? <Separator /> : null}
        {filtered.map((option) => <button key={option.id} disabled={disabled || creating} type="button" className="flex min-h-11 items-center gap-3 rounded-xl px-2.5 text-left hover:bg-carbon/5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-carbon/20" aria-pressed={value === option.id} onClick={() => selectOption(option.id)}><EntityOptionIcon option={option} /><span className="min-w-0 flex-1 truncate text-[13px] font-semibold">{option.name}</span></button>)}
        {!filtered.length && !onCreate ? <p className="m-0 py-5 text-center text-sm text-carbon/50">No hay resultados.</p> : null}
      </div>
      {createError ? <p role="alert" className="m-0 mt-2 text-xs font-semibold text-zivelo">{createError}</p> : null}
      {onCreate && query.trim() && !exactMatch ? <><Separator className="my-2" /><Button type="button" variant="ghost" className="w-full justify-start" disabled={disabled || creating} onClick={() => void create()}><Plus data-icon="inline-start" aria-hidden="true" />{creating ? "Creando…" : `Crear “${query.trim()}”`}</Button></> : null}
    </PopoverContent>
  </Popover>;
}

    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button type="button" variant="outline" className={cn("w-full justify-start", className)} aria-label={`Seleccionar ${label.toLowerCase()}`}>
          ...
          <span className="truncate">{selected?.name ?? `Seleccionar ${label.toLowerCase()}`}</span>
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-[min(340px,calc(100vw-32px))]">
        <PopoverHeader>
          <PopoverTitle>Seleccionar {label.toLowerCase()}</PopoverTitle>
          <PopoverDescription>Busca por nombre o usuario.</PopoverDescription>
        </PopoverHeader>
        ...
        <div className="mt-2 flex max-h-64 flex-col gap-1 overflow-y-auto overscroll-contain">
          {filtered.map((user) => (
            <button
              key={user.id}
              type="button"
              className="flex min-h-11 items-center gap-3 rounded-xl px-2.5 text-left transition-colors hover:bg-carbon/5 focus-visible:outline focus-visible:outline-2 focus-visible:outline-zivelo"
              onClick={() => { onValueChange(user.id); setOpen(false); setQuery(""); }}
            >
              <UserAvatar user={user} />
              <span className="min-w-0 flex-1">
                <span className="block truncate text-sm font-semibold">{user.name}</span>
                {user.username ? <span className="block truncate text-xs text-carbon/45">@{user.username}</span> : null}
              </span>
              {value === user.id ? <Check aria-hidden="true" /> : null}
            </button>
          ))}

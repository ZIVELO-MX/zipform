...
export function SlideOver({ open, title, children, footer, onOpenChange, className }: SlideOverProps) {
  ...
  return (
    <dialog
      ...
      className={cn(
        "fixed bottom-0 left-auto right-0 top-0 m-0 h-dvh max-h-none w-[min(520px,calc(100vw-48px))] max-w-none overscroll-contain border-0 bg-[#FAFAF9] p-0 text-carbon shadow-[-8px_0_32px_rgba(29,29,27,0.10)] backdrop:bg-carbon/50 open:animate-in open:slide-in-from-right-6 open:duration-200",
        className
      )}
    >
      <div className="flex h-full flex-col border-l border-carbon/10">
        <header className="flex shrink-0 items-center justify-between border-b border-carbon/10 px-5 py-4">
          <h2 className="m-0 text-sm font-bold uppercase tracking-normal text-carbon/75">{title}</h2>
          <form method="dialog">

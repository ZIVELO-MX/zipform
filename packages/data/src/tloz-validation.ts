...
function required(value: string | undefined, field: string, label: string, fields: Record<string, string>, min = 1) {
  if (!value?.trim() || value.trim().length < min) fields[field] = `${label} debe tener al menos ${min} caracteres.`;
}
...
export function validateMissionCreate(input: TlozMissionCreateInput) {
  ...
  required(input.icon, "icon", "El icono", fields);
  if (input.title.trim().length > 160) fields.title = "El título no puede superar 160 caracteres.";
  if (input.description.length > 5000) fields.description = "La descripción no puede superar 5000 caracteres.";
  dates(input.startDate, input.dueDate, fields); finish(fields); return { ...input, title: input.title.trim(), description: input.description.trim() };
}

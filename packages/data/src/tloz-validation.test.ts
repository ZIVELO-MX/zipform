...
describe("TLOZ creation validation", () => {
  it("rejects incomplete entities before persistence", () => {
    ...
    expect(() => validateQuestItemCreate({ name: "Key", description: "", icon: "KeyRound", color: "#2D6CDF", status: "unlocked", category: "access" })).toThrow(TlozValidationError);
  });
  ...
  it("requires a valid color for inventory items", () => {
    expect(() => validateQuestItemCreate({ name: "Key", description: "", icon: "KeyRound", color: "blue", status: "locked", category: "access" })).toThrow(TlozValidationError);
  });
